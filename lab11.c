...
#define MAX_FILE_SIZE 512
  ...
  int msg_size = read_all_bytes(message_path, message, MAX_FILE_SIZE);
  int sig_size = read_all_bytes(sign_path, signature, MAX_FILE_SIZE);
  ...
  EVP_DigestVerifyUpdate(ctx, message, msg_size);
  int result = EVP_DigestVerifyFinal(ctx, signature, sig_size);
  EVP_MD_CTX_free(ctx);
